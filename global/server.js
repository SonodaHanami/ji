...
const app = createApp({
    ...
    methods: {
        ...
        fetchData: async function() {
            ...
            let start = new Date();
            await axios.get('https://yubo.run/arknights/arknights_dictionary/event/global.json').then(res => {
                let elapsed = +new Date() - start;
                ElementPlus.ElMessage({
                    message: `请求事件数据完成，用时${elapsed}ms`,
                    type: 'success',
                });
                console.log(`fetchData: arknights_dictionary/event/global.json ${elapsed} ms`)
            ...
            await axios.post('https://yubo.run/api/arknights/get_ji_stats_global', payload).then(res => {
                if (res.data.code != 0) {
        ...
        init_latest_data: function() {
            for (let rogue_id in this.charts) {
                ...
                let latest_day_str = this.ji_stats_date ? toLocalISOStringPDT(this.ji_stats_date, true, 10) : Object.keys(this.ji_stats_data[rogue_id]).slice(-1)[0];
        ...
        check_data_change: function(rogue_id, timestamp, prev_data, curr_data) {
            if (prev_data['character'] && curr_data['character']) {
                for (let job in prev_data['character']) {
                    ...
                    if (is_changed) {
                        ...
                        // this.data_change_log.push(`${toLocalISOStringPDT(new Date(timestamp * 1000))} ${ROGUE_NAMES[rogue_id]} 在完成结局时，出场率最高的${JOBS[job]}干员 [${prev_str}] -> [${curr_str}]`);
                        let isoStringLength = new Date() - new Date(timestamp * 1000) <= 7 * 86400000 ? 16 : 10;
                        this.data_change_log.push({
                            'time': toLocalISOStringPDT(new Date(timestamp * 1000), is_28_hours=true, length=isoStringLength),
            ...
            if (prev_data['candle'] && curr_data['candle']) {
                ...
                for (let i = 0; i < prev_data['candle'].length; i++) {
                    if (prev_data['candle'][i]['name'] != curr_data['candle'][i]['name']) {
                        is_changed = true;
                        // this.data_change_log.push(`${toLocalISOStringPDT(new Date(timestamp * 1000))} ${ROGUE_NAMES[rogue_id]} candle ${i + 1} ${prev_data['candle'][i]['name']} -> ${curr_data['candle'][i]['name']}`);
                    }
                }
                if (is_changed) {
                    for (let i = 0; i < prev_data['candle'].length; i++) {
                    ...
                    // this.data_change_log.push(`${toLocalISOStringPDT(new Date(timestamp * 1000))} ${ROGUE_NAMES[rogue_id]} 被授予伺烛客身份最多次的干员 [${prev_str}] -> [${curr_str}]`);
                    let isoStringLength = new Date() - new Date(timestamp * 1000) <= 7 * 86400000 ? 16 : 10;
                    this.data_change_log.push({
                        'time': toLocalISOStringPDT(new Date(timestamp * 1000), is_28_hours=true, length=isoStringLength),
                        'rogue_name': ROGUE_NAMES[rogue_id],
            ...
            if (prev_data['expeditionGuide'] && curr_data['expeditionGuide']) {
                ...
                if (is_changed) {
                    ...
                    // this.data_change_log.push(`${toLocalISOStringPDT(new Date(timestamp * 1000))} ${ROGUE_NAMES[rogue_id]} 累计入卷最多次的干员 [${prev_str}] -> [${curr_str}]`);
                    let isoStringLength = new Date() - new Date(timestamp * 1000) <= 7 * 86400000 ? 16 : 10;
                    this.data_change_log.push({
                        'time': toLocalISOStringPDT(new Date(timestamp * 1000), is_28_hours=true, length=isoStringLength),
                        'rogue_name': ROGUE_NAMES[rogue_id],
        ...
        is_date_disabled: function(date) {
            const date_str = toLocalISOStringPDT(new Date(date), true, 10);
        ...
        init_chart_line: function(rogue_id, chart_name) {
            ...
            // let today_str = toLocalISOStringPDT(today);
            let data_list = [];
            // let x_axis_list = [];
            ...
            for (let i = 0; i < date_list.length; i++) {
                ...
                for (let j = 0; j < timestamp_list.length; j++) {
                    if (+new Date() - timestamp_list[j] * 1000 < timestamp_delta) {
                        ...
                        // x_axis_list.push(toLocalISOStringPDT(new Date(this.ji_stats_data[rogue_id][date_list[i]][timestamp_list[j]]['last_update_timestamp'] * 1000)));
                    }
                }
            }
            // let delta_list = [0];
            ...
            var option = {
                ...
                tooltip: {
                        ...
                        label: {
                            ...
                            formatter: function(params) {
                                ...
                                if (params.axisDimension === 'x') {
                                    ...
                                    return toLocalISOStringPDT(date, true, 16);
                                }
                    ...
                    formatter: function(params) {
                        ...
                        const dateStr = toLocalISOStringPDT(date, true, 16);
                        ...
                        result += `<span>服务器时间</span><span style="float:right;margin-left:20px;font-size:14px;color:#6d6e73;font-weight:900">${toLocalISOStringPDT(date, false, 16)}</span><br/>`
                ...
                xAxis: [
                    {
                        ...
                        axisLabel: {
                            ...
                            formatter: function(object) {
                                return toLocalISOStringPDT(new Date(object), true, 16);
                            },
                        },
                        axisPointer: {
                            type: 'shadow',
                ...
                series: [
                    ...
                    },
                    {
                        ...
                        //         // return EVENTS[toLocalISOStringPDT(new Date(object.value[0]))];
                        //     }
                        // },
                        tooltip: {
                            show: false,
                        ...
                        markArea: {
                            ...
                            emphasis: {
                                ...
                                label: {
                                    ...
                                    position: 'inside',
                                    formatter: function (object) {
                                        return `${object.data.name}\n(${toLocalISOStringPDT(new Date(object.data.coord[0][0]))} ~ ${toLocalISOStringPDT(new Date(object.data.coord[1][0]))})`;
                                    },
                                    fontSize: 16,
                                    fontWeight: 'bold',
                                    color: '#325',
            ...
            const option = {
                        ...
                        label: {
                            ...
                            formatter: function(params) {
                                ...
                                if (params.axisDimension === 'x') {
                                    // x轴的值格式化
                                    const date = new Date(params.value);
                                    return toLocalISOStringPDT(date, true, 10);
                                }
                                return params.value;
                            },
                        }
                ...
                xAxis: {
                    ...
                    axisLabel: {
                        interval: 'auto',  // 自动计算间隔
                        formatter: function(object) {
                            return toLocalISOStringPDT(new Date(object), true, 10);
                        },
                    },
                    axisPointer: {
                        type: 'shadow',
                            ...
                            emphasis: {
                                ...
                                label: {
                                    ...
                                    formatter: function (object) {
                                        ...
                                        return `${object.data.name} 第${object.data.rank}名\n${day_count}天 (${toLocalISOStringPDT(start, true, 10)} ~ ${toLocalISOStringPDT(end, true, 10)})`;
                                    },
                                    fontSize: 20,
                                    fontWeight: 'bold',
                                    color: '#325',
