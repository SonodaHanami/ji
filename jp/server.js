...
const CHART_NAMES = {
    ...
    'costHp': 'スポット「易与」にてジーから商品を購入するために消費した耐久値の総数',
    // 'nodePass': '累计在岁兽残识中消耗的烛火',
    'nodePass': '歳獣の残識にて消費された燭火の総数',
    'fall': '博士烛火尚未燃尽就被迫回到人间的次数',
    'fall': '燭火が残っている状態で歳獣の残識から追放された総回数',
    'rotate': '界园的供游客盖章收藏的宝印被博士转动过的次数',
    'rotate': '観光客が印影を集める際の宝印をドクターが回転させた総回数',
    'gold': '累计通过坎诺特印记获得源石锭',
    'gold': 'キャノットの印によって獲得した源石錐の総数',
    'raidian': '在岁兽残识中遭遇同心事件时，由电弧带领进入玉石同心球中的小镇的次数',
    'raidian': '歳獣の残識での遭遇「同心」にて、レイディアンを玉の中の村に進入させた総回数',
    'character': '在完成结局时，出场率最高的干员',
    'character': '結末にたどり着いた際、最も多くその場に居合わせたオペレーター',
    'wrath': '完成探险时，最常显露的岁时',
    'wrath': '結末にたどり着いた際、最も多く発動された上位三種の歳時',
    // 'candle': '被授予伺烛客身份最多次的干员',
    'candle': '伺燭客になった回数が最も多く上位のオペレーターたち',
...
const app = createApp({
    ...
    methods: {
        ...
        fetchData: async function() {
            ...
            let start = new Date();
            await axios.get('https://yubo.run/arknights/arknights_dictionary/event/jp.json').then(res => {
                let elapsed = +new Date() - start;
                ElementPlus.ElMessage({
                    message: `请求事件数据完成，用时${elapsed}ms`,
                    type: 'success',
                });
                console.log(`fetchData: arknights_dictionary/event/jp.json ${elapsed} ms`)
            ...
            await axios.post('https://yubo.run/api/arknights/get_ji_stats_jp', payload).then(res => {
                if (res.data.code != 0) {
        ...
        init_latest_data: function() {
            for (let rogue_id in this.charts) {
                ...
                let latest_day_str = this.ji_stats_date ? toLocalISOStringTokyo(this.ji_stats_date, true, 10) : Object.keys(this.ji_stats_data[rogue_id]).slice(-1)[0];
        ...
        check_data_change: function(rogue_id, timestamp, prev_data, curr_data) {
            if (prev_data['character'] && curr_data['character']) {
                for (let job in prev_data['character']) {
                    ...
                    if (is_changed) {
                        ...
                        // this.data_change_log.push(`${toLocalISOStringTokyo(new Date(timestamp * 1000))} ${ROGUE_NAMES[rogue_id]} 在完成结局时，出场率最高的${JOBS[job]}干员 [${prev_str}] -> [${curr_str}]`);
                        let isoStringLength = new Date() - new Date(timestamp * 1000) <= 7 * 86400000 ? 16 : 10;
                        this.data_change_log.push({
                            'time': toLocalISOStringTokyo(new Date(timestamp * 1000), is_28_hours=true, length=isoStringLength),
            ...
            if (prev_data['candle'] && curr_data['candle']) {
                ...
                for (let i = 0; i < prev_data['candle'].length; i++) {
                    if (prev_data['candle'][i]['name'] != curr_data['candle'][i]['name']) {
                        is_changed = true;
                        // this.data_change_log.push(`${toLocalISOStringTokyo(new Date(timestamp * 1000))} ${ROGUE_NAMES[rogue_id]} candle ${i + 1} ${prev_data['candle'][i]['name']} -> ${curr_data['candle'][i]['name']}`);
                    }
                }
                if (is_changed) {
                    for (let i = 0; i < prev_data['candle'].length; i++) {
                    ...
                    // this.data_change_log.push(`${toLocalISOStringTokyo(new Date(timestamp * 1000))} ${ROGUE_NAMES[rogue_id]} 被授予伺烛客身份最多次的干员 [${prev_str}] -> [${curr_str}]`);
                    let isoStringLength = new Date() - new Date(timestamp * 1000) <= 7 * 86400000 ? 16 : 10;
                    this.data_change_log.push({
                        'time': toLocalISOStringTokyo(new Date(timestamp * 1000), is_28_hours=true, length=isoStringLength),
                        'rogue_name': ROGUE_NAMES[rogue_id],
            ...
            if (prev_data['expeditionGuide'] && curr_data['expeditionGuide']) {
                ...
                if (is_changed) {
                    ...
                    // this.data_change_log.push(`${toLocalISOStringTokyo(new Date(timestamp * 1000))} ${ROGUE_NAMES[rogue_id]} 累计入卷最多次的干员 [${prev_str}] -> [${curr_str}]`);
                    let isoStringLength = new Date() - new Date(timestamp * 1000) <= 7 * 86400000 ? 16 : 10;
                    this.data_change_log.push({
                        'time': toLocalISOStringTokyo(new Date(timestamp * 1000), is_28_hours=true, length=isoStringLength),
                        'rogue_name': ROGUE_NAMES[rogue_id],
        ...
        is_date_disabled: function(date) {
            const date_str = toLocalISOStringTokyo(new Date(date), true, 10);
        ...
        init_chart_line: function(rogue_id, chart_name) {
            ...
            // let today_str = toLocalISOStringTokyo(today);
            let data_list = [];
            // let x_axis_list = [];
            ...
            for (let i = 0; i < date_list.length; i++) {
                ...
                for (let j = 0; j < timestamp_list.length; j++) {
                    if (+new Date() - timestamp_list[j] * 1000 < timestamp_delta) {
                        ...
                        // x_axis_list.push(toLocalISOStringTokyo(new Date(this.ji_stats_data[rogue_id][date_list[i]][timestamp_list[j]]['last_update_timestamp'] * 1000)));
                    }
                }
            }
            // let delta_list = [0];
            ...
            var option = {
                ...
                tooltip: {
                        ...
                        label: {
                            ...
                            formatter: function(params) {
                                ...
                                if (params.axisDimension === 'x') {
                                    ...
                                    return toLocalISOStringTokyo(date, true, 16);
                                }
                    ...
                    formatter: function(params) {
                        ...
                        const dateStr = toLocalISOStringTokyo(date, true, 16);
                        ...
                        result += `<span>服务器时间</span><span style="float:right;margin-left:20px;font-size:14px;color:#6d6e73;font-weight:900">${toLocalISOStringTokyo(date, false, 16)}</span><br/>`
                ...
                xAxis: [
                    {
                        ...
                        axisLabel: {
                            ...
                            formatter: function(object) {
                                return toLocalISOStringTokyo(new Date(object), true, 16);
                            },
                        },
                        axisPointer: {
                            type: 'shadow',
                ...
                series: [
                    ...
                    },
                    {
                        ...
                        //         // return EVENTS[toLocalISOStringTokyo(new Date(object.value[0]))];
                        //     }
                        // },
                        tooltip: {
                            show: false,
                        ...
                        markArea: {
                            ...
                            emphasis: {
                                ...
                                label: {
                                    ...
                                    position: 'inside',
                                    formatter: function (object) {
                                        return `${object.data.name}\n(${toLocalISOStringTokyo(new Date(object.data.coord[0][0]))} ~ ${toLocalISOStringTokyo(new Date(object.data.coord[1][0]))})`;
                                    },
                                    fontSize: 16,
                                    fontWeight: 'bold',
                                    color: '#325',
            ...
            const option = {
                        ...
                        label: {
                            ...
                            formatter: function(params) {
                                ...
                                if (params.axisDimension === 'x') {
                                    // x轴的值格式化
                                    const date = new Date(params.value);
                                    return toLocalISOStringTokyo(date, true, 10);
                                }
                                return params.value;
                            },
                        }
                ...
                xAxis: {
                    ...
                    axisLabel: {
                        interval: 'auto',  // 自动计算间隔
                        formatter: function(object) {
                            return toLocalISOStringTokyo(new Date(object), true, 10);
                        },
                    },
                    axisPointer: {
                        type: 'shadow',
                            ...
                            emphasis: {
                                ...
                                label: {
                                    ...
                                    formatter: function (object) {
                                        ...
                                        return `${object.data.name} 第${object.data.rank}名\n${day_count}天 (${toLocalISOStringTokyo(start, true, 10)} ~ ${toLocalISOStringTokyo(end, true, 10)})`;
                                    },
                                    fontSize: 20,
                                    fontWeight: 'bold',
                                    color: '#325',
